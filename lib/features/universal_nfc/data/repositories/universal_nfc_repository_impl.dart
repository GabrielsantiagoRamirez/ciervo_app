...
class UniversalNfcRepositoryImpl implements UniversalNfcRepository {
  ...
  Future<Result<List<SavedPaymentMethod>>> paymentMethods() => _guard(
    () async =>
        (await _remote.paymentMethods()).map((dto) => dto.toDomain()).toList(),
  );
  ...
  Future<Result<UniversalNfcPayment>> payment(String paymentIntentId) =>
      _guard(() async => (await _remote.payment(paymentIntentId)).toDomain());

  @override
  Future<Result<UniversalNfcPayment>> confirm(String paymentIntentId) =>
      _guard(() async => (await _remote.confirm(paymentIntentId)).toDomain());

  @override
  Future<Result<void>> cancel(String paymentIntentId) =>
      _guard(() async => _remote.cancel(paymentIntentId));

  @override
  Future<Result<List<KidsNfcParentApproval>>> kidsApprovals() => _guard(
    () async =>
        (await _remote.kidsApprovals()).map((dto) => dto.toDomain()).toList(),
  );
  ...
  Future<Result<UniversalNfcPayment>> approveKidsPayment(
    ...
    () async => (await _remote.approveKidsPayment(paymentIntentId)).toDomain(),
  );
